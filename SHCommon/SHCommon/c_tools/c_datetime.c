...
static long _findNextLeapYear(long year){
    if(_isLeapYear(year)) return year;
    if(_isLeapYear(year + 1)) return year + 1;
    if(_isLeapYear(year + 2)) return year + 2;
    if(_isLeapYear(year + 3)) return year + 3;
    return -1;
}

static long _findPrevLeapYear(long year){
    if(_isLeapYear(year)) return year;
    if(_isLeapYear(year - 1)) return year - 1;
    if(_isLeapYear(year - 2)) return year - 2;
    if(_isLeapYear(year - 3)) return year - 3;
...
static int _isLeapDay(SHDateTime *dt){
...
int addDaysToTimestamp(long timestamp,long days, TimeAdditionOptions options,long *ans){
    (void)options;
...
int addMonthsToTimestamp(long timestamp,long months, TimeAdditionOptions options
  ,long *ans){
    SHDateTime dt;
    if(timestampToDateObj(timestamp,0,&dt)) return -1;
    int exMonths = months % 12;
    long results;
    long years = months/12;
    addYearsToTimestamp(timestamp,years,options,&results);
    int daySum = _monthSums[exMonths -1];
    addDaysToTimestamp(results,daySum,0,ans);
    return 0;
}

int addYearsToTimestamp(long timestamp,long years, TimeAdditionOptions options
  ,long *ans){
    SHDateTime dt;
    if(years == 0){
        *ans = timestamp;
    ...
    if(options & KEEP_DAY_ERROR){
        if(!_isLeapYearFromBaseYear(yearSum)&&_isLeapDay(&dt)){
            return -1;
        }
        dt.year = yearSum;
    ...
    if((options & KEEP_DAY_BKW) || (options & KEEP_DAY_FWD)){
        if(_isLeapYearFromBaseYear(yearSum)&&_isLeapDay(&dt)){
            if(options & KEEP_DAY_BKW) dt.day = 28;
            else if(options & KEEP_DAY_FWD){
                dt.day = 1;
                dt.month = 3;
            }
        }
    ...
    if(options & STRICT_DAY){
        timestamp += years*UNIX_YEAR;
        *ans = timestamp;
        return 0;
    }
    long nextLeapYear = _findNextLeapYear(dt.year);
    long prevLeapYear = _findPrevLeapYear(dt.year);
    if(yearSum < nextLeapYear && yearSum > prevLeapYear){
        dt.year = yearSum;
        if(dateObjToTimestamp(&dt,ans)) return -1;
        return 0;
    }
    long offsetYears = years > 0? yearSum - nextLeapYear:prevLeapYear - yearSum;
    long leapCount = offsetYears / LEAP_CYCLE_YEAR;
    dt.year = yearSum;
    long tmp;
    dateObjToTimestamp(&dt,&tmp);
    addDaysToTimestamp(tmp,leapCount,0,ans);
    return 0;
}
