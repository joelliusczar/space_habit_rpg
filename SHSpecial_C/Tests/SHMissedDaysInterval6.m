...
@interface SHMissedDaysInterval6 : XCTestCase

@end

@implementation SHMissedDaysInterval6
...
-(void)testMissedDaysInterval6 {
	...
	dec		25	26	27	28	29	30	01	1	2
		...
		*/
		...
		//Tuesday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 5, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Wednesday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 6, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Thursday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 7, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Friday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 8, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Saturday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 9, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Sunday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 10, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Monday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 11, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Tuesday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 12, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Wednesday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 13, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Thursday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 14, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Friday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 15, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Saturday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 16, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Sunday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 17, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Monday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 18, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Tuesday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 19, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Wednesday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 20, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Thursday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 21, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Friday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 22, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Saturday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 23, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Sunday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 24, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 24);

		
		//Monday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 25, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 25);

		
		//Tuesday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 26, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 26);

		
		//Wednesday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 27, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 27);

		
		//Thursday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 28, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 28);

		
		//Friday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 29, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 29);

		
		//Saturday
		testDate = (struct SHDatetime){.year = 2018, .month = 6, .day = 30, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 30);

		
		//Sunday
		testDate = (struct SHDatetime){.year = 2018, .month = 7, .day = 1, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 31);

		
		//Monday
		testDate = (struct SHDatetime){.year = 2018, .month = 7, .day = 2, .timezoneOffset = -18000};
		status = SH_missedDays(&testDate, &testContext, &result);
		XCTAssertEqual(result, 31);
